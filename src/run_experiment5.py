...
from src.data.vocab import build_vocab_with_freqs
...
from src.training.trainer_exp5 import train_exp5
...
EPOCHS     = int(os.environ.get("RECIPE1M_EPOCHS",        "10"))
BATCH_SIZE = 128
LR         = 1e-5   # lower than LoRA — full visual encoder is being updated
TOP_K      = 5
...
def build_loaders(device: str) -> tuple[list[str], list[int], DataLoader, DataLoader]:
    train_recipes = load_recipes(
        DET_INGRS, LAYER1, LAYER2, IMAGE_ROOT,
    ...
    vocab, freqs = build_vocab_with_freqs(
        train_recipes, top_n=200, min_freq=2, exclude=NON_VISUAL
    )
    ...
    return vocab, freqs, train_loader, test_loader
...
def main() -> None:
    ...
    vocab, freqs, train_loader, test_loader = build_loaders(device=device)
    clip = CLIPWrapper(device=device)

    print("=== Experiment 5: Asymmetric Frequency-Weighted Fine-tuning ===")
    ...
    print("Training with asymmetric ingredient loss...")
    losses = train_exp5(
        clip=clip,
    ...
    print(
        f"{'Asym fine-tuned':<22} "
        f"{after['P@1']:.3f}  {after['P@5']:.3f}  {after['R@5']:.3f}  {after['F1@5']:.3f}"
    )

    os.makedirs("outputs", exist_ok=True)
    results = {
        "experiment": "experiment5_asymmetric_ingredient_loss",
    ...
    with open("outputs/experiment5_results.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)

    print("\nResults saved to outputs/experiment5_results.json")
